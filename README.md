...
This module improves on this approach by executing `kubectl` commands with a [kubernetes job](https://registry.terraform.io/providers/hashicorp/kubernetes/latest/docs/resources/job_v1). The job will either replace the `aws-auth` with new configmap managed with terraform or it will patch in-place.
